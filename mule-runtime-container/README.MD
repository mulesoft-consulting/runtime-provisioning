...
The following guidelines are intended for devops team / developers working on automating MuleSoft Runtime Provisioning.
The first section provides information about the Mule Runtime Dockerization. 
The second second section provides information about hybrid servers automaetd regitsration in Anypoint Runtime Manager (ARM), and automated cluster creation.
The scripts provided can be used both for ARM registration in the US or EU management plane.

## Dockerizing Mule Runtime EE
...
* MuleSoft Runtime Zip file: 3.8.X / 3.9.x. 
** Name it *after mule-ee-distribution-standalone-VERSION* 
...
1. Run:
```$> ./build-container.sh {image_tag} ```

    Example:
        ```
        BA-JPORCEL-OSX:mule-runtime-container-385 juanporcel$ ./build-container.sh 1.0.5
        Docker version 17.12.0-ce, build c97c6d6
        Image Id: mule-runtime-3.8.5:1.0.5
        Sending build context to Docker daemon  381.1MB
        Step 1/21 : FROM openjdk:8
        ---> 891c9734d5ab
        Step 2/21 : MAINTAINER Bank Of Ireland, Republic of Ireland
        ---> Using cache
        ---> 3ba5815e0acb
        Step 3/21 : ENV MULE_VERSION=3.8.5
        ---> Using cache
        ---> 3b51de035ecb
        Step 4/21 : COPY ./mule-ee-distribution-standalone-$MULE_VERSION.zip /opt/
        COPY failed: stat /var/lib/docker/tmp/docker-builder355292189/mule-ee-distribution-standalone-3.8.5.zip: no such file or directory
        BA-JPORCEL-OSX:mule-runtime-container-385 juanporcel$
        ```
    ...
    Example:
        ...
        mule-runtime-3.8.5                    1.0.6               a65b9b5e523b        17 seconds ago      1.82GB
        ```

### Create the Container
1. Run: ```sudo ./run-container.sh {image_tag}```

    Example:
        ```
        BA-JPORCEL-OSX:mule-runtime-container-385 juanporcel$ sudo ./run-container.sh 1.0.7
        13ac1e2c66ba679abae7e0479a83d6225cfb8c3708bece46aa51f484b5edb2cf
        CONTAINER ID        IMAGE                         COMMAND                  CREATED                  STATUS                    PORTS                                                                                NAMES
        13ac1e2c66ba        mule-ee-runtime-3.8.5:1.0.7   "/opt/mule/bin/mule …"   Less than a second ago   Up Less than a second     443/tcp, 1098/tcp, 5000/tcp, 5701/tcp, 8091/tcp, 54327/tcp, 0.0.0.0:8081->8081/tcp   mule-ee-runtime-3.8.5-1.0.7
        ```
...
1. Remove Docker Image: ```$> docker image rm --force {repository}:{tag} ```

    Example:
        ```$> docker image rm --force mule-ee-runtime-3.8.5:1.0.9 ```
        
2. Verify the Image has been effectively removed: ```docker image ls```
...
```docker exec -it {mule_container_name} /opt/mule/bin/arm_register_server.sh  -u {username} -p {password} -o {orgName} -e {envName} -s {serverName} -c {clusterName}```

```docker exec -it {mule_container_name} /opt/mule/bin/mule restart```

**Important.** To register the server in the EU Management Plane (eu1 region), add the ```--region eu1``` parameter.

#### Example

```docker exec -it {mule_container_name} /opt/mule/bin/arm_register_server.sh --region eu1 -u {username} -p {password} -o {orgName} -e {envName} -s {serverName} -c {clusterName}```
...
{"data":{"id":99007,"timeCreated":1525295609350,"timeUpdated":1525295609350,"name":"jmporcel-test-cluster-8","type":"CLUSTER","status":"RUNNING","multicastEnabled":false,"primaryNodeId":null,"servers":[{"id":99006,"timeCreated":1525295580195,"timeUpdated":1525295609354,"name":"jmporcel-local-node-a-8","type":"SERVER","serverType":"GATEWAY","muleVersion":"3.8.5","gatewayVersion":"3.8.5","agentVersion":"1.9.4","licenseExpirationDate":1668470400000,"status":"RUNNING","addresses":[{"ip":"172.17.0.2","networkInterface":"eth0"}],"clusterId":99007,"clusterName":"jmporcel-test-cluster-8","serverIp":"10.8.8.51","runtimeInformation":{"jvmInformation":{"runtime":{"name":"OpenJDK Runtime Environment","version":"1.8.0_162-8u162-b12-1~deb9u1-b12"},"specification":{"vendor":"Oracle Corporation","name":"Java Platform API Specification","version":"1.8"}},"osInformation":{"name":"Linux","version":"4.9.87-linuxkit-aufs","architecture":"amd64"},"muleLicenseExpirationDate":1668470400000}}],"visibilityMap":{"mapNodes":[{"serverId":99006,"visibleNodeIds":[],"unknownNodeIps":[]}]}}}Done.
...
### Related assets
These assets are based on the following resources:
1. https://wiki.corp.mulesoft.com/display/SVCS/Dockerizing+Mule+ESB
2. https://wiki.corp.mulesoft.com/display/SVCS/Register+Mule+runtime+in+ARM+programatically
